...
@selector(
    uses={
        json_filter, met_filters, trigger_selection, mc_weight,
        pdf_weights, murmuf_weights, pu_weight, process_ids, cutflow_features,
        increment_stats, attach_coffea_behavior, gen_lep_tau,
    },
    produces={
        trigger_selection, mc_weight,
        pdf_weights, murmuf_weights, pu_weight, process_ids, cutflow_features,
...
def onlyMET(
    ...
    **kwargs,
    ...
    if self.dataset_inst.is_mc:
        ...
        # groups
        print("weight_map created")
        # results.x.n_central_jets=ak.num(events.Jet.pt, axis=1)
        group_map = {
            **group_map,
            # per process
            "process": {
    ...
    print("increment_stats done")

    events = self[gen_lep_tau](events, **kwargs)
    print("gen_lep_tau done")
