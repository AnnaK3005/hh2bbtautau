...
ele_pt_neg_hh_mean= np.mean(ele_pt_neg_hh)
ele_pt_neg_hh_std= np.std(ele_pt_neg_hh)
ele_pt_neg_hh_standardized= (ele_pt_neg_hh - ele_pt_neg_hh_mean) / ele_pt_neg_hh_std

ele_pt_neg_tt_mean= np.mean(ele_pt_neg_tt)
ele_pt_neg_tt_std= np.std(ele_pt_neg_tt)
ele_pt_neg_tt_standardized= (ele_pt_neg_tt - ele_pt_neg_tt_mean) / ele_pt_neg_tt_std



def get_MET_pt(data):
    met_pt=data.MET.pt
    charge_array=data.ElectronFromTau.charge
    flat_charge_array=ak.flatten(charge_array, axis=-1)
    no_empty=flat_charge_array>=-1
    flat_charge_array=flat_charge_array[no_empty]
    negative_mask= flat_charge_array < 0
    positive_mask= flat_charge_array > 0
    met_pt_neg=met_pt[negative_mask]
    met_pt_pos=met_pt[positive_mask]
    
    return met_pt_neg, met_pt_pos

met_pt_neg_hh, met_pt_pos_hh = get_MET_pt(data_hh_ggf)
met_pt_neg_tt, met_pt_pos_tt = get_MET_pt(data_tt_dl)

met_pt_neg_hh_mean= np.mean(met_pt_neg_hh)
met_pt_neg_hh_std= np.std(met_pt_neg_hh)
met_pt_neg_hh_standardized= (met_pt_neg_hh - met_pt_neg_hh_mean) / met_pt_neg_hh_std

met_pt_neg_tt_mean= np.mean(met_pt_neg_tt)
met_pt_neg_tt_std= np.std(met_pt_neg_tt)
met_pt_neg_tt_standardized= (met_pt_neg_tt - met_pt_neg_tt_mean) / met_pt_neg_tt_std
...
gen_z_neg_e_hh_ggf_plus_one = add_target_to_array(gen_z_neg_e_hh_ggf, ele_pt_neg_hh, met_pt_neg_hh, hh_weights_neg, target_value=0)
gen_z_neg_e_tt_dl_plus_zero = add_target_to_array(gen_z_neg_e_tt_dl, ele_pt_neg_tt, met_pt_neg_tt, tt_dl_weights_neg, target_value=1)
...
z_array, output_array, pt_array, met_pt_array, weights = np.split(shuffled_array, 5, axis=1)

input_array= np.column_stack((z_array,pt_array, met_pt_array))
...
epochs=100
model_name = f"gen_model_5_layers_10_nodes_{epochs}_epochs_with_pt_and_METpt"
...
output_path = os.path.join(
    thisdir,
    'dnn_models', 'plots', 'gen_model_with_pt_and_METpt', 'ROC_plots',
    model_name
)
draw_roc(
    y_test = y_test,
...
output_path = os.path.join(
    thisdir,
    'dnn_models', 'plots', 'gen_model_with_pt_and_METpt', 'ROC_plots', model_name +
    "_pt"
)
...
def plot_loss():
    ...
    plt.savefig('/afs/desy.de/user/k/kindsvat/Documents/hh2bbtautau/hbt/ml/dnn_models/plots/gen_model_with_pt_and_METpt/loss_and_accuracy/gen_model_'+ model_name +"_loss_and_val_loss")
...
def plot_accuracy():
    ...
    plt.legend(loc='upper left')
    plt.savefig('/afs/desy.de/user/k/kindsvat/Documents/hh2bbtautau/hbt/ml/dnn_models/plots/gen_model_with_pt_and_METpt/loss_and_accuracy/gen_model_'+ model_name +"_binary_accuracy_and_val_binary_accuracy")
...
for label, data, truth_labels in zip(
    ...
    for mask_value in np.unique(truth_labels):
        ...
        output_folder = os.path.join(thisdir, 'dnn_models', 'output_gen_model_with_pt_and_METpt')
