...
data_path = {
"hh_ggf_bbtautau":["/nfs/dust/cms/user/kindsvat/cf_cache/hbt_store/analysis_hbt/cf.UniteColumns/run2_2017_nano_uhh_v11/hh_ggf_bbtautau_madgraph/nominal/calib__default/sel__default/prod__default/no_reco_selections/data_0.parquet"],
...
def get_pt(particle_pt, count_mask, variable, particle_charge):
    ...
    positive_mask = charge_array > 0
    negative_mask = charge_array < 0
    valid_entries = ak.any(positive_mask, axis=1) & ak.any(~positive_mask, axis=1)
    
    pt = pt[count_mask]
    pt = pt[two_values_mask]
    pt = pt[~three_values]
    pt_pos = pt[positive_mask]
    pt_pos = pt_pos[valid_entries]
    pt_pos = ak.flatten(pt_pos, axis=-1)
    pt_neg = pt[negative_mask]
    pt_neg = pt_neg[valid_entries]
    pt_neg = ak.flatten(pt_neg, axis=-1)
    
    return pt_pos, pt_neg

ele_pos_pt_hh, ele_neg_pt_hh = get_pt(data_hh_ggf.ElectronFromTau["pt"], count_hh_mask_ele, gen_z_e_hh, data_hh_ggf.ElectronFromTau["charge"])
ele_pos_pt_tt, ele_neg_pt_tt = get_pt(data_tt_dl.ElectronFromTau["pt"], count_tt_mask_ele, gen_z_e_tt, data_tt_dl.ElectronFromTau["charge"])

mu_pos_pt_hh, mu_neg_pt_hh = get_pt(data_hh_ggf.MuonFromTau["pt"], count_hh_mask_mu, gen_z_m_hh, data_hh_ggf.MuonFromTau["charge"])
mu_pos_pt_tt, mu_neg_pt_tt = get_pt(data_tt_dl.MuonFromTau["pt"], count_tt_mask_mu, gen_z_m_tt, data_tt_dl.MuonFromTau["charge"])


def get_eta(particle_eta, count_mask, variable, particle_charge):
    ...
    eta = eta[count_mask]
    eta = eta[two_values_mask]
    eta = eta[~three_values]
    eta_pos = eta[positive_mask]
    eta_pos = eta_pos[valid_entries]
    eta_pos = ak.flatten(eta_pos, axis=-1)
    eta_neg = eta[negative_mask]
    eta_neg = eta_neg[valid_entries]
    eta_neg = ak.flatten(eta_neg, axis=-1)
    
    return eta_pos, eta_neg

ele_pos_eta_hh, ele_neg_eta_hh = get_pt(data_hh_ggf.ElectronFromTau["eta"], count_hh_mask_ele, gen_z_e_hh, data_hh_ggf.ElectronFromTau["charge"])
ele_pos_eta_tt, ele_neg_eta_tt = get_pt(data_tt_dl.ElectronFromTau["eta"], count_tt_mask_ele, gen_z_e_tt, data_tt_dl.ElectronFromTau["charge"])

mu_pos_eta_hh, mu_neg_eta_hh = get_pt(data_hh_ggf.MuonFromTau["eta"], count_hh_mask_mu, gen_z_m_hh, data_hh_ggf.MuonFromTau["charge"])
mu_pos_eta_tt, mu_neg_eta_tt = get_pt(data_tt_dl.MuonFromTau["eta"], count_tt_mask_mu, gen_z_m_tt, data_tt_dl.MuonFromTau["charge"])
...
gen_z_e_hh_plus_zero = add_target_to_array(#gen_z_neg_e_hh, 
                                           #gen_z_pos_e_hh, 
                                           ele_pos_pt_hh, 
                                           ele_neg_pt_hh, 
                                           ele_pos_eta_hh, 
                                           ...
                                           hh_weights_ele, 
                                           target_value=0)
gen_z_e_tt_plus_one = add_target_to_array(#gen_z_neg_e_tt, 
                                          #gen_z_pos_e_tt, 
                                          ele_pos_pt_tt, 
                                          ele_neg_pt_tt, 
                                          ele_pos_eta_tt, 
...
gen_z_m_hh_plus_zero = add_target_to_array(#gen_z_neg_m_hh, 
                                           #gen_z_pos_m_hh, 
                                           mu_pos_pt_hh, 
                                           mu_neg_pt_hh,
                                           mu_pos_eta_hh, 
                                           ...
                                           hh_weights_mu, 
                                           target_value=0)
gen_z_m_tt_plus_one = add_target_to_array(#gen_z_neg_m_tt, 
                                          #gen_z_pos_m_tt, 
                                          mu_pos_pt_tt, 
                                          mu_neg_pt_tt,
                                          mu_pos_eta_tt, 
...
combined_array = np.concatenate((gen_z_e_hh_plus_zero, gen_z_e_tt_plus_one, gen_z_m_hh_plus_zero, gen_z_m_tt_plus_one))
...
epochs=100
model_name = f"det_model_{epochs}_epochs_ele_and_mu_only_pt_and_eta"
...
output_path = os.path.join(
    thisdir,
    'dnn_models', 'plots', 'det_model_ele_and_mu_only_pt_and_eta', 'ROC_plots',
    model_name
)
draw_roc(
    y_test = y_test,
...
output_path = os.path.join(
    thisdir,
    'dnn_models', 'plots', 'det_model_ele_and_mu_only_pt_and_eta', 'ROC_plots', model_name +
    "_energy_fractions"
)
...
def plot_loss():
    ...
    plt.savefig('/afs/desy.de/user/k/kindsvat/Documents/hh2bbtautau/hbt/ml/dnn_models/plots/det_model_ele_and_mu_only_pt_and_eta/loss_and_accuracy/gen_model_'+ model_name +"_loss_and_val_loss")
...
def plot_accuracy():
    ...
    plt.legend(loc='upper left')
    plt.savefig('/afs/desy.de/user/k/kindsvat/Documents/hh2bbtautau/hbt/ml/dnn_models/plots/det_model_ele_and_mu_only_pt_and_eta/loss_and_accuracy/gen_model_'+ model_name +"_binary_accuracy_and_val_binary_accuracy")
...
for label, data, truth_labels in zip(
    ...
    for mask_value in np.unique(truth_labels):
        ...
        output_folder = os.path.join(thisdir, 'dnn_models', 'output_det_model_ele_and_mu_only_pt_and_eta')
